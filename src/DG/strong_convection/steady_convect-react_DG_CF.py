...
if(__name__ == "__main__"):
    ...
    u = conv_react_CF.u
    u_ex = conv_react_CF.u_exact

    # Print error
    printMPI(f"Error L2: {np.sqrt(assemble_scalar(form((u-u_ex)**2*dx))): .2e}")
    printMPI(f"Error CF: {np.sqrt(assemble_scalar(form(np.max([conv_react_CF.mu, conv_react_CF.conv]) * (u-u_ex)**2*dx + 1/2*abs(inner(conv_react_CF.beta, FacetNormal(conv_react_CF.mesh)))*(u-u_ex)**2*ds))): .2e}")
    ...
    # otherwise create interactive plot
    if pyvista.OFF_SCREEN:
        figure = plotter.screenshot(f"./{base_name_save}_u_CF_nx-{int(p.nx)}.png", transparent_background=True)
                
    comm.Barrier()
    if rank == 0:
        img = Image.open(f"./{base_name_save}_u_CF_nx-{int(p.nx)}.png")
        width, height = img.size
        # Setting the points for cropped image
        left = 0.3 * width
        top = 0.15 * height
        ...
        bottom = 0.9 * height
        im_cropped = img.crop((left, top, right, bottom)) # default window size is 1024x768
        im_cropped.save(f"./{base_name_save}_u_CF_nx-{int(p.nx)}.png")
    else:
        plotter.show()
    plotter.close()